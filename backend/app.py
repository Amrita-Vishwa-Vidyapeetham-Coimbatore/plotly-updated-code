...
MONGODB_URI = os.getenv('MONGODB_URI', 'mongodb://localhost:27017/Seismic-Project')
...
def extract_segy_from_gridfs_zip(file_id, extract_to):
    """Extract SEGY files from ZIP archive stored in GridFS"""
    segy_files = []
    try:
        zip_data = fs.get(file_id).read()
        ...
        print(f"Error extracting ZIP file from GridFS: {e}")
    
    return segy_files

def save_file_to_gridfs(file, filename):
    ...
    try:
        ...
        print(f"Saved {filename} to GridFS with ID: {file_id}")
        return file_id
    except Exception as e:
    ...
    try:
        for file in files:
            if file and allowed_file(file.filename):
                ...
                if not file_id:
                    return jsonify({'error': f'Failed to save {filename} to database'}), 500
                
                uploaded_file_ids.append({'filename': filename, 'file_id': str(file_id)})
                
                if filename.lower().endswith('.zip'):
                    extracted_files = extract_segy_from_gridfs_zip(file_id, temp_dir)
                    segy_files.extend(extracted_files)
                else:
                    gridfs_file = get_file_from_gridfs(file_id)
                    if gridfs_file:
                        temp_segy_path = os.path.join(temp_dir, filename)
                        with open(temp_segy_path, 'wb') as f:
                            f.write(gridfs_file.read())
                        segy_files.append(temp_segy_path)
        ...
        if success:
            ...
            if cube_info:
                return jsonify({
                    ...
                    'files': [f['filename'] for f in uploaded_file_ids],
...
if __name__ == '__main__':
    ...
    print("File storage: MongoDB GridFS")
